...
func (m *coreModuleRuntime) Namespace() starlark.Value {
	return starlarkstruct.FromStringDict(starlark.String(model.ModuleSourceCore), starlark.StringDict{
...
func (m *coreModuleRuntime) BuildComponents(_ model.DesiredModel) ([]model.ComponentSpec, error) {
	components := make([]model.ComponentSpec, 0, len(m.factDecls)+len(m.structuredFileDecls)+len(m.synthesisHookDecls)+len(m.bootstrapRequirementDecls)+len(m.pythonDependencyDecls)+len(m.taskDecls)+len(m.taskSurfaceDecls)+len(m.triggerBindingDecls))
			...
			Facts: []model.FactProviderSpec{
				{
					...
					Values:         model.CloneNestedMap(decl.values),
				},
			},
		})
	}
	...
	for _, decl := range m.structuredFileDecls {
		...
		ownedPaths, err := filefmt.InferOwnedPaths(decl.format, decl.values)
		if err != nil {
		...
		components = append(components, model.ComponentSpec{
			...
			Files: []model.StructuredFileSpec{
				{
					Path:          model.JoinProjectPath(project.EffectivePath, decl.path),
					Format:        decl.format,
					OwnedPaths:    ownedPaths,
					DesiredValues: model.CloneNestedMap(decl.values),
				},
			},
		})
	}
	...
	for _, decl := range m.synthesisHookDecls {
		components = append(components, model.ComponentSpec{
			...
			Version:         model.CoreModuleVersion,
			TargetProjectID: decl.targetProjectID,
			SynthesisHooks: []model.SynthesisHookSpec{
				{
					Phase:           decl.phase,
					TargetProjectID: decl.targetProjectID,
	...
	for _, decl := range m.bootstrapRequirementDecls {
		components = append(components, model.ComponentSpec{
			...
			Version:         model.CoreModuleVersion,
			TargetProjectID: decl.targetProjectID,
			BootstrapRequirements: []model.BootstrapRequirementSpec{
				{
					Tool:            decl.tool,
					TargetProjectID: decl.targetProjectID,
	...
	for _, decl := range m.pythonDependencyDecls {
		components = append(components, model.ComponentSpec{
			...
			RoutedIntents: []model.RoutedIntentSpec{
				{
					Kind:            model.IntentPythonDependencyRequest,
					Capability:      model.CapabilityPythonPackageManager,
					TargetProjectID: decl.targetProjectID,
					Attributes: map[string]string{
						model.IntentAttrRequirement: decl.requirement,
					},
				},
			},
		})
	...
	for _, decl := range m.taskSurfaceDecls {
		components = append(components, model.ComponentSpec{
			...
			Version:         model.CoreModuleVersion,
			TargetProjectID: decl.targetProjectID,
			TaskSurfaces: []model.TaskSurfaceSpec{
				{
					Name:            decl.name,
					Kind:            decl.kind,
					TargetProjectID: decl.targetProjectID,
	...
	for _, decl := range m.triggerBindingDecls {
		components = append(components, model.ComponentSpec{
			...
			Version:         model.CoreModuleVersion,
			TargetProjectID: decl.targetProjectID,
			TriggerBindings: []model.TriggerBindingSpec{
				{
...
func (m *coreModuleRuntime) project(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	var kind string = model.ProjectKindGeneric
	var name string
	var path string = "."
	var id string
...
func (m *coreModuleRuntime) jsonFile(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	return m.declareStructuredFile("json_file", model.FileFormatJSON, args, kwargs)
}
...
func (m *coreModuleRuntime) tomlFile(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	return m.declareStructuredFile("toml_file", model.FileFormatTOML, args, kwargs)
}
...
func (m *coreModuleRuntime) yamlFile(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	return m.declareStructuredFile("yaml_file", model.FileFormatYAML, args, kwargs)
}
...
func isKnownSynthesisPhase(value string) bool {
	switch value {
	case model.SynthesisPhasePre, model.SynthesisPhaseMain, model.SynthesisPhasePost:
		return true
	default:
		return false
	}
